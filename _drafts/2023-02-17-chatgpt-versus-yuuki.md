...
To install XShogi on MacOS Monterey, install Homebrew and run `brew install xshogi`.  However, when I ran `xshogi`, I got "Error: Can't open display: ".  I had to install XQuartz with `brew install --cask xquartz`, launch it from Launchpad, go to Applications > Customize, press Add Item, and add the `xshogi` command with the name "XShogi" (or whatever).  Finally, go to Applications > XShogi to launch it.

[^1]: The log uploaded to Pastebin was generated by running the following JavaScript code in the Brave (i.e., Chromium) console on the chat page.  As of ChatGPT Feb 13 version, it renders the conversation in div elements with the `whitespace-pre-wrap` class.  (This may not work in the future.)
